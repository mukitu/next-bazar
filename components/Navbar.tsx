  ...
  return (
    <>
            ...
            <div className="hidden md:flex items-center gap-10">
              ...
              {user ? (
                <div className="flex items-center gap-6">
                  <button onClick={() => onNavigate('orders')} className="text-[11px] font-black text-slate-500 hover:text-orange-500 uppercase tracking-widest transition">Orders</button>
                  <button onClick={signOut} className="text-[11px] font-black text-red-500 uppercase tracking-widest hover:text-red-700 transition">Logout</button>
                </div>
              ) : (
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 z-50 mobile-bottom-nav px-8 py-4 flex justify-between items-center shadow-[0_-10px_30px_rgba(0,0,0,0.05)] rounded-t-[2rem]">
        ...
        <button onClick={() => onNavigate('orders')} className={`flex flex-col items-center gap-1 ${currentPage === 'orders' ? 'text-orange-500' : 'text-slate-300'}`}>
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
          <span className="text-[9px] font-black uppercase">Orders</span>
        </button>
        <button onClick={() => user ? onNavigate('orders') : onNavigate('login')} className={`flex flex-col items-center gap-1 ${currentPage === 'login' ? 'text-orange-500' : 'text-slate-300'}`}>
