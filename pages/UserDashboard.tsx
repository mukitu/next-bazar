...
const UserDashboard: React.FC = () => {
  ...
  const handleProfileUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Identity data synchronized successfully!' });
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="text-center animate-pulse">
        ...
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Syncing Command Center...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12 md:py-20 animate-fadeIn pb-32">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
        <div>
          <h1 className="text-4xl md:text-5xl font-black text-slate-900 uppercase tracking-tighter italic">User <span className="text-orange-500">Dashboard</span></h1>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 ml-1">Member Since: {user?.created_at ? new Date(user.created_at).getFullYear() : '2024'}</p>
        </div>
      ...
      {activeTab === 'orders' ? (
          ...
          {orders.length === 0 ? (
            <div className="bg-white p-20 rounded-[3rem] text-center border-2 border-dashed border-slate-100">
               <div className="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mx-auto mb-6">
                 <svg className="w-8 h-8 text-slate-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
               </div>
               <p className="text-slate-300 font-black uppercase tracking-widest text-xs italic">Your commerce journey is waiting to begin.</p>
            </div>
          ) : (
            orders.map(order => {
                  ...
                  <div className="p-8 md:p-10">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center font-black text-slate-300 text-[10px]">#{order.id.slice(0,4)}</div>
                        <div>
                          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Transaction Reference</p>
                      ...
                      <div className="flex gap-3 w-full md:w-auto">
                        ...
                        <button 
                          ...
                          className="flex-1 md:flex-none bg-slate-900 text-white px-6 py-2.5 rounded-full text-[9px] font-black uppercase tracking-widest hover:bg-orange-600 transition shadow-lg active:scale-95"
                        >
                    ...
                    {order.status !== 'Cancelled' && (
                      <div className="relative mb-12 px-2">
                        ...
                        <div className="flex justify-between mt-4 text-[7px] font-black text-slate-300 uppercase tracking-tighter">
                          <span>Initiated</span>
                          <span>Verified</span>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pt-8 border-t border-slate-50">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-800 uppercase">{order.payment_method}</p>
                      </div>
                      <div className="col-span-2 text-right">
                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Assets Value</p>
                    ...
                    {isExpanded && (
                      <div className="mt-10 pt-10 border-t border-slate-50 animate-fadeIn space-y-6">
                        <div className="bg-slate-50 p-6 rounded-3xl">
                           <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Destination Terminal</p>
                           <p className="text-xs font-bold text-slate-700 leading-relaxed uppercase italic">{order.shipping_address}</p>
                        </div>
                        <div className="space-y-3">
                          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Asset Inventory ({order.order_items?.length})</p>
                          {order.order_items?.map((item: any) => (
                            <div key={item.id} className="flex items-center gap-4 bg-white p-4 rounded-2xl border border-slate-50">
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-[9px] text-slate-400 font-bold uppercase">{item.quantity} Unit × {CURRENCY_SYMBOL}{item.price_at_time}</p>
                              </div>
        ...
        <div className="max-w-2xl mx-auto animate-fadeIn">
          ...
          <div className="bg-white p-10 md:p-14 rounded-[3.5rem] shadow-xl border border-slate-100 relative overflow-hidden">
            ...
            <form onSubmit={handleProfileUpdate} className="space-y-10 relative z-10">
              <div className="flex flex-col gap-2 mb-4">
                <h3 className="text-xl font-black uppercase italic tracking-tighter text-slate-900">Synchronize <span className="text-orange-500">Identity</span></h3>
                <p className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">Your primary shipping and contact credentials</p>
              </div>
              ...
              <div className="space-y-6">
                <div className="group">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-50 border-2 border-transparent rounded-2xl p-5 text-sm font-black focus:ring-0 focus:border-orange-500/30 focus:bg-white transition-all uppercase italic outline-none shadow-inner" 
                    placeholder="e.g. Abdullah Rahman"
                  />
                </div>

                <div className="group">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3 block ml-1 group-focus-within:text-orange-500 transition-colors">Hotline (Mobile Number)</label>
                  <input 
                    type="tel" 
                    required
                    value={profileForm.phone}
                    onChange={e => setProfileForm({...profileForm, phone: e.target.value})}
                    className="w-full bg-slate-50 border-2 border-transparent rounded-2xl p-5 text-sm font-black focus:ring-0 focus:border-orange-500/30 focus:bg-white transition-all outline-none shadow-inner" 
                    placeholder="01XXXXXXXXX"
                  />
                </div>

                <div className="group">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3 block ml-1 group-focus-within:text-orange-500 transition-colors">Saved Shipping Terminal (Address)</label>
                  <textarea 
                    rows={4}
                    required
                    value={profileForm.address}
                    onChange={e => setProfileForm({...profileForm, address: e.target.value})}
                    className="w-full bg-slate-50 border-2 border-transparent rounded-2xl p-5 text-sm font-black focus:ring-0 focus:border-orange-500/30 focus:bg-white transition-all uppercase leading-relaxed outline-none shadow-inner resize-none" 
              ...
              <div className="pt-6">
                ...
                >
                  {updating ? 'PROCESSING SYNC...' : 'AUTHORIZE UPDATES'}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="mt-8 text-center">
            <p className="text-[9px] font-bold text-slate-300 uppercase tracking-widest italic">Encrypted Secure Data Management System</p>
          </div>
        </div>
      )}
    </div>
